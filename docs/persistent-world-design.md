...
- **v0.39** — Per-game route scoping (multi-game assumption removed). All player-facing game-state endpoints are now explicitly `gameId`-scoped. **Removed routes**: `POST /api/action`, `GET /api/diplomacy`, `GET /api/war/:warId/council`, `GET /api/world` (all return 404 — update any curl bookmarks). **New player routes**: `POST /api/games/:id/action`, `GET /api/games/:id/diplomacy`, `GET /api/games/:id/war/:warId/council` (identical logic to removed routes but membership-authenticated and game-scoped). **Legacy-world unchanged**: still accessible as `gameId='legacy-world'`; player1–5 via `PLAYER_NATION_MAP` as before — the only change is that the UI and API now use `/api/games/legacy-world/*` paths. **Admin route map** (old → new): `/api/admin/world-full` → `/api/admin/games/:gameId/world-full`; `/api/admin/tick` → `/api/admin/games/:gameId/tick`; `/api/admin/set-phase` → `/api/admin/games/:gameId/set-phase`; `/api/admin/reset-world` → `/api/admin/games/:gameId/reset-world`. **Dev route map** (old → new): `/api/dev/tick` → `/api/dev/games/:gameId/tick`; `/api/dev/set-phase` → `/api/dev/games/:gameId/set-phase`; `/api/dev/reset-world` → `/api/dev/games/:gameId/reset-world` (old `/api/dev/*` player1-gated routes removed). **`getSession` shim removed**: the `GameMembership` fallback that resolved `nationId` from the most recent active membership is gone; `getSession` now does only `getSessionNationId(token)`. **Frontend**: `InfoPanel`, `DiplomacyPanel`, `WarCouncilPanel`, `DevToolbar` all accept `gameId` prop and call scoped endpoints; `App.tsx` passes `gameId` from route params (defaults to `'legacy-world'` for unparameterized use); `api.ts` diplomatic helpers (`proposeTreaty`, `acceptTreaty`, etc.) now require `gameId` as first argument. **New `GameSwitcher` component** (`web/src/components/GameSwitcher.tsx`): renders a compact top bar listing all the user's games; hidden when ≤1 game; clicking navigates to the appropriate `/games/:id/*` route based on game status; renders in `GamePlayScreen` above `PrestigeLeaderboard`. 31/31 harness scenarios byte-identical (engine untouched). Tagged `v0.39-complete`.

- **v0.38** — Minimal lobby/auth/territory-selection UI. **Scope**: browser-only flows for account creation, login/logout, lobby browsing/creation/joining, game start, and territory selection — no curl required. **Styling note**: clean dark theme (`#111827` bg, `#1f2937` surfaces, `#3b82f6` accent) with no custom illustration, icons, or thematic chrome. Phase 8 will introduce a full visual identity pass with sourced/generated assets and potentially a different design language — this session's styling uses exclusively inline React `style` props and a shared `styles.ts` token file, making it easy to replace wholesale. **Dependencies**: added `react-router-dom@6` to `web/package.json`. **New files** (`web/src/`): `AuthContext.tsx` (auth provider + `useAuth` hook; session managed by httpOnly cookie server-side; `localStorage` stores username for display only; mounts with a `/api/me` probe to confirm cookie validity); `styles.ts` (shared design tokens + common style objects); `screens/LoginScreen.tsx` (username/password → `POST /api/auth/login`, redirect `/lobby`; link to register); `screens/RegisterScreen.tsx` (→ `POST /api/auth/register`, redirect `/login?registered=1`); `screens/LobbyScreen.tsx` (two-column: create-game form + browse/join list; logout button; polls `GET /api/games`); `screens/GameLobbyScreen.tsx` (member slot grid, host start panel with empty-slot policy picker, leave button; polls 3s; auto-redirects to `/games/:id/select-territory` or `/games/:id/play` on status change); `screens/TerritorySelectionScreen.tsx` (split-panel: left = roll/reroll/confirm controls + candidate cards showing name/qualityTier/coastal; right = MapLibre dark map with candidates highlighted in 3 distinct colors, rest de-emphasized; snipe message shown inline; other-players' status bar; host force-resolve button; polls 3s; auto-redirects to `/games/:id/play` on transition). **`api.ts`** additions: 16 new methods (`register`, `authLogin`, `authLogout`, `createGame`, `listGames`, `getGame`, `joinGame`, `leaveGame`, `startGame`, `endGame`, `rollTerritories`, `getCandidates`, `rerollTerritories`, `confirmTerritory`, `forceResolve`) + new types (`GameListItem`, `GameDetail`, `GameMember`, `CandidateView`, `CandidatesResponse`, `ConfirmResponse`). **`App.tsx`**: rewritten as React Router `<Routes>` tree with `RequireAuth` guard and `RootRedirect`; legacy map view extracted to `GamePlayScreen` at `/games/:id/play`; routing documentation comment at top. **`main.tsx`**: wraps non-admin app in `<BrowserRouter>` + `<AuthProvider>`. **Server change** (`server/src/index.ts`): `GET /api/games/:id` extended to return `hostUserId`, `aiSlots`, `removedSlots`, and `confirmedTerritoryId` on each member (needed by lobby detail + territory selection UI). **Route map**: `/` → redirects; `/login` + `/register` → public; `/lobby` + `/games/:id` + `/games/:id/select-territory` + `/games/:id/play` → auth-required. 31/31 harness scenarios pass byte-identical (frontend-only change). Tagged `v0.38-complete`.

- **v0.37** — Random starter territory selection. **Phase flow**: `lobby → territory_selection → active`. Host starting a game now transitions to `territory_selection` instead of directly to `active`; world init and tick scheduler are deferred until all players confirm. **Schema**: `Game` model gained `territorySelectionStartedAt DateTime?` (set when host starts; AFK deadline = `lastTickAt + tickIntervalSeconds`); `GameMembership` gained `rerollUsed Boolean @default(false)` and `confirmedTerritoryId String?`; new `TerritoryCandidate` model (`id`, `gameId`, `userId`, `slotIndex 0–2`, `territoryId`, `confirmed`, `createdAt`). Migration `20260611000000_territory_selection`. **New module** `server/src/territorySelection.ts`: inverse-weight draw (`weight = 1/(qualityTier + 0.1)` — lower-tier territories drawn ~3× more often than tier-3, prevents clustering at 16 best starts); `MID_HIGH_THRESHOLD = 2` (qualityTier ≥ 2 = 34/36 territories) — guarantees at least one candidate per player meets the threshold (safety net against all-tier-1 draw); `rollCandidates(gameId, userId, defs)` draws 3 from pool minus already-confirmed-by-others, applies threshold guarantee, persists rows; `confirmCandidate(gameId, userId, slotIndex, defs)` does snipe-check (if territory was confirmed by another player between roll and confirm, returns `sniped:true` with fresh auto-rerolled candidates); `autoAssignUnconfirmed(gameId, defs)` picks highest-tier candidate per unconfirmed player (snipe retry ≤ 2), used by both AFK deadline and force-resolve; `transitionToActive(gameId, defs, memberships)` calls `ensureGameWorldInitialized(gameId, defs, capitalsBySlot)` then arms tick scheduler. **World init change**: `ensureGameWorldInitialized` signature extended to `(gameId, defs, capitalsBySlot: Record<number, string> = {})` — uses player-confirmed territory as nation capital when present, falls back to `SLOT_CAPITALS` round-robin for AI slots. **Scheduler changes**: `scheduleSelectionDeadline(gameId, defs, delayMs)` arms deadline timer using same `gameTimers` map (deregisterGame cancels it too); on fire, lazily imports `autoAssignUnconfirmed` to avoid circular dependency then calls it; `resumeActiveGames` extended to also resume `territory_selection` games by re-arming the deadline timer from remaining `lastTickAt` delta. **Start endpoint change**: `POST /api/games/:id/start` now sets `status='territory_selection'`, `lastTickAt=now()`, and calls `scheduleSelectionDeadline` — does NOT init world or arm tick scheduler. **5 new territory endpoints** in `server/src/index.ts`: `POST /api/games/:id/territory/roll` (draw 3 candidates, reject if already confirmed); `GET /api/games/:id/territory/candidates` (view current candidates, `rerollUsed`, `confirmedCount/totalHuman`); `POST /api/games/:id/territory/reroll` (one-time; full re-roll of all 3, marks `rerollUsed`); `POST /api/games/:id/territory/confirm` (`{slotIndex}`, snipe check, triggers `transitionToActive` if last confirmation); `POST /api/games/:id/territory/force-resolve` (host-only; cancels deadline timer, calls `autoAssignUnconfirmed`). **Verified behaviors**: (1) Both players roll — qualityTiers shown for all 6 candidates, at least one ≥ threshold per player; (2) Reroll replaces all 3 candidates, `rerollUsed` set; (3) Snipe scenario: second confirmer of same territory receives `{sniped:true, candidates:[...fresh]}` with auto-rerolled pool; (4) Both confirm → `transitioned:true`, DB nations have player-confirmed territories as capitals; (5) Force-resolve on a game with one AFK player (no roll, no confirm): auto-assigns highest-tier available territory, transitions to active with correct capitals. All 31 harness scenarios pass byte-identical (territory selection is pre-game setup, doesn't touch `resolveTick`). Tagged `v0.37-complete`.

- **v0.36** — Lobby lifecycle + per-instance tick workers. **Schema**: Extended `Game` model: `status` default changed `'active'` → `'lobby'`; `hostUserId String?` → `Int?` (FK to `User`); added `endedAt DateTime?`, `endReason String?` (`'win_condition' | 'host_ended'`), `removedSlots Json @default("[]")`, `aiSlots Json @default("[]")`. New `GameMembership` model: `id`, `gameId`, `userId`, `slotIndex`, `nationId?`, `joinedAt`; `@@unique([gameId, userId])` + `@@unique([gameId, slotIndex])`. New `FastForwardVote` model: `id`, `gameId`, `userId`, `tickNumber`, `votedAt`; `@@unique([gameId, userId])`. Migration `20260610030000_lobby_system`. **New module** `server/src/scheduler.ts`: `gameTimers Map<string, Timeout>` in-process per-game tick chain; `scheduleGameTick(gameId, defs, delayMs)` sets and re-arms timeouts; `runGameTick(gameId, defs)` wraps full atomic-transaction tick pipeline (same defense-pact, mandate-refund, caretaker, AI logic as legacy) scoped to one gameId; win-condition check post-tick via `computeDominantNations()`; `deregisterGame(gameId)` clears timeout; `resumeActiveGames(defs)` called at server startup to re-arm all `status='active'` games computing remaining delay from `lastTickAt`. **World init**: `loadWorldState` and `saveWorldState` gained optional `gameId` parameter; when non-legacy, all DB queries filter by `gameId`, territory row IDs stored as `gameId::defId`, prefix stripped on load so engine always sees raw `def.id` names. `ensureGameWorldInitialized(gameId, defs)` creates a full fresh world: per-game `WorldMeta` row (next integer id, `gameId` field set), one nation per non-removed slot prefixed `gameId_slot_i`, territory/army rows with prefixed IDs. **Lobby endpoints** (`server/src/index.ts`): `POST /api/games` (authenticated; creates lobby + host membership); `GET /api/games` (list lobby-status games); `GET /api/games/:id` (detail with membership list + tick); `POST /api/games/:id/join`; `POST /api/games/:id/leave`; `POST /api/games/:id/start` (host-only; resolves empty slots via `emptySlotPolicy: 'open' | 'removed' | 'ai'` + per-slot overrides; calls `ensureGameWorldInitialized`; calls `scheduleGameTick`); `POST /api/games/:id/end` (host-only; sets `status='ended'`; calls `deregisterGame`). **Fast-forward** endpoints: `POST /api/games/:id/fast-forward/vote` (upserts vote; if all human players voted fires tick immediately via `setImmediate`, clears votes, re-arms scheduler); `GET /api/games/:id/fast-forward/status` (`{votes, required, ready}`). Fast-forward denominator = human player memberships excluding ai/removed slots. **Session B scope complete**: legacy-world game and its cron scheduler unchanged; new games fully isolated; `resumeActiveGames` called after `startScheduler` on startup. All 31 harness scenarios pass byte-identical (scheduler wraps `resolveTick` — doesn't change it). Tagged `v0.36-complete`.

- **v0.35** — North America territory geometry fix (map dissolve). **Root cause:** `scripts/generate-adjacency.mjs` `mergeGeometries()` concatenated individual US state and Canadian province polygons into a flat `MultiPolygon` without dissolving shared borders. MapLibre rendered each sub-polygon separately, showing all 50 US state borders and provincial borders visually on the map. `usa_south` had 42 sub-polygons (12 states + offshore islands), `canada_northwest` had 105 (Nunavut archipelago). The macro-region game territories existed correctly in `engine/src/data/americas.json`, `americas-adjacency.json`, and as GeoJSON features — but their geometries were the wrong kind of MultiPolygon. **Fix:** Added `import { union } from '@turf/union'` (already in `node_modules`) and a `dissolveGeometries(features)` helper that passes all features as a `FeatureCollection` to `@turf/union`, producing a properly unioned geometry with internal borders dissolved. Added a parallel `dissolvedGeometries` map that is populated alongside the existing `geometries` map (adjacency detection continues using the un-dissolved MultiPolygon, which is fine for `booleanIntersects`). The GeoJSON map output section now uses `dissolvedGeometries` for US territories and `dissolveGeometries()` for Canada sub-regions. **Results:** `usa_midwest` 12 sub-polygons → 1 `Polygon` (contiguous states merge completely); `usa_northeast` 25 → 11 (remaining = coastal islands); `usa_south` 42 → 30 (remaining = Florida Keys, barrier islands, Texas islands); `usa_west` 34 → 24 (remaining = Hawaii + Pacific islands); Canada sub-regions similarly reduced; `canada_northwest` 105 → 98 (Nunavut islands are genuinely disconnected). Script re-run; `scripts/data/americas-map.geojson` and `web/public/territories.geojson` regenerated. No changes to `engine/src/data/americas.json`, adjacency data, game state, or any engine/server files. All 31 harness scenarios pass byte-identical. Tagged `v0.35-complete`.
- **v0.34** — Multi-world data model + auth foundation. **Schema**: All 21 game-state tables (`WorldMeta`, `Nation`, `TerritoryState`, `QueuedAction`, `EventLog`, `Proposal`, `Treaty`, `War`, `Army`, `TerritoryModifier`, `BorderSkirmish`, `TreatyHistory`, `TerritoryClaim`, `Federation`, `InstantTrade`, `TradeRoute`, `WarCouncil`, `PrestigeHistory`, `Embassy`, `TradeRouteAgreement`, `TradeShipment`) gained `gameId String @default("legacy-world")` + `@@index([gameId])`. New `Game` model (`id`, `name`, `hostUserId`, `maxPlayers`, `status`, `tickIntervalSeconds`, `lastTickAt`). Migration `20260610020000_multi_world_auth` backfills existing rows to `gameId='legacy-world'` and inserts a legacy-world `Game` row. **Auth**: New `User` model (`id`, `username`, `passwordHash`, `createdAt`) and `UserSession` model (`id`, `userId`, `token`, `nationId`, `createdAt`, `expiresAt`). `bcryptjs` (pure-JS, cost=12) replaces plaintext PLAYERS map in `server/src/auth.ts`. Five player1–5 `User` rows created at startup with real bcrypt hashes; migration uses identifiable placeholder hashes (`$2b$12$placeholder…`) replaced by `ensureUsersInitialized` on first boot. Session tokens: 64-char hex, stored in `UserSession` table, looked up on every authenticated request; 30-day TTL. `getSession` split into sync `getSessionToken` + async DB-lookup `getSession`. New endpoints: `POST /api/auth/register` (username 3–32 chars, password min 4), `POST /api/auth/login` (identical behavior to existing `/api/login`), `POST /api/auth/logout`. Existing `/api/login` and `/api/logout` updated to call `loginUser`/`logoutUser`. **Quality tier**: `qualityTier?: 1 | 2 | 3` added to `TerritoryDef` interface in `engine/src/types.ts` (optional read-only metadata, no simulation effect). Formula: `score = basePopulation×0.4 + baseIndustry×0.35 + baseWealth×0.25 + (isCoastal ? 1.5 : 0)`; tier 3 ≥ 8.0, tier 2 ≥ 5.0, tier 1 < 5.0 [thresholds PLACEHOLDER]. All 36 territories in `engine/src/data/americas.json` tagged: 18 tier-3 (score ≥ 8.0, all coastal), 16 tier-2 (5.0–7.99), 2 tier-1 (`brazil_amazonia` 4.05, `peru_selva` 4.05). Admin endpoint: `GET /api/admin/territory/:id/quality-tier` returns tier + score + components + thresholds. **Harness**: Pre-existing TS errors in `runner.ts` (lines 661, 795) fixed with proper type casts; 31/31 scenarios pass byte-identical. Tagged `v0.34-complete`.
- **v0.33** — Trade Routes — Growth, Decay, and Cultural Feedback (systems-backlog §11). New `TradeRouteAgreement` table (distinct from legacy `TradeRoute`) with `TradeShipment` transit model. Three route types: domestic (same nation, Mandate-gated, no treaty), international market-tier (land border, `trade_route` clause), international port-tier (sea crossing, port ≥ L1, `trade_route` clause). Growth: `currentCapacity += baseCapacity × 0.05` per shipment arrival, capped at `baseCapacity × 1.5`; upkeep `currentCapacity × 0.1` per tick. Loss event on territory ownership change, treaty non-renewal, or infra removal: `TerritoryModifier` unrest spike `(lostValue / growthCap) × 0.1` for 5 ticks on endpoint territories; event log entry with percentage grown and cycle count. Cultural feedback: `merchantPressure` toward individualist; `routeCountPressure` adds isolationist entanglement (separate counter from treaty count — no double-count). Prestige: `tradeRouteScore × PRESTIGE_PER_TRADE_CAPACITY (0.3)` (server-side only). Renewal carry-over: renewed `trade_route` clause treaties preserve `currentCapacity` and `cyclesCompleted` — the grown state is a real asset. Schema: `portLevel INT DEFAULT 1` on `TerritoryState`; new `TradeRouteAgreement` and `TradeShipment` tables; migration `20260610010000_trade_routes`. Server: `establishTradeRoute.ts` action handler; `trade_route` clause support in `proposeTreaty`, `acceptTreaty`, `proposeRenewal`; `_renewalFromClauseId` metadata carries prior route ID through the renewal proposal. Harness: `establish_trade_route`, `renew_trade_route`, `assert_route_capacity`, `assert_route_cycles`, `assert_route_upkeep` action types; `trade-routes.csv` output. Three new scenarios (31 total): `trade-route-growth` (capacity growth + upkeep scaling over 30 ticks), `trade-route-loss` (ownership change → loss event + unrest spike), `trade-route-renewal` (renewal preserves grown state). All 28 prior scenarios byte-identical. Tagged `trade-routes-complete`.
- **v0.32** — Step 0: isCoastal audit + build_market system. **isCoastal corrections:** 12 territories in `engine/src/data/americas.json` corrected from `false → true` after explicit audit of real-world geography vs. derived values: `usa_west, canada_west, canada_central, canada_northwest, mexico_norte, mexico_centro, guatemala, honduras, ecuador, peru_costa_sierra, argentina_pampa_norte, argentina_patagonia`. Confirmed 6 truly inland territories: `usa_midwest, colombia_orinoquia, brazil_amazonia, peru_selva, bolivia, paraguay`. **build_market action:** inland equivalent of build_port — mutually exclusive with port, same mandate cost (2) and industry cost (5 [PLACEHOLDER]), same build time (3 ticks [PLACEHOLDER]). DB migration adds `hasMarket BOOLEAN NOT NULL DEFAULT false` to `TerritoryState`. Prisma schema + engine types + loader `initialState` updated. Action handler `buildMarket.ts` validates `!isCoastal` (reject with "Territory is coastal — build a port instead") and `!hasPort`. Engine `tick.ts`: `BUILD_TICKS['market']`, `BUILD_INDUSTRY['market']`, `build_market` action case, construction completion sets `hasMarket = true`. **Trade capacity:** `LAND_MARKET_CAPACITY_MULTIPLIER = 1.5 [PLACEHOLDER]` — applied multiplicatively on top of road multiplier when both endpoints have road+market (effective: base × 1.5 × 1.5 = 2.25×). Market without road does not activate the bonus. **Mandate budget:** "developed" formula updated throughout: `road + (port OR market) + fort L1+` using Prisma `OR: [{hasPort:true},{hasMarket:true}]` queries. **Prestige infra score:** `(hasPort || hasMarket ? 1 : 0)`. **AI doctrine:** `scoreAction('build_market')` same formula as `build_port`, gated to inland territories only. **Admin:** `POST /api/admin/territory/:id/toggle-market` endpoint; admin panel shows `Mkt✓/Mkt✗` for non-coastal territories. **Player UI:** InfoPanel shows Market row (non-coastal only), Build Market button, and `hasMarket` wired into `TerritoryView`. **Validation:** (1) `build_market` on `costa_rica` (coastal) → `"Territory is coastal — build a port instead"`; (2) `build_market` on `usa_midwest` (inland) → `{"ok":true}`; (3) `computeTradeCapacity(road+market, road+market, false)` → `22.5` vs `15` road-only. All 28 harness scenarios byte-identical.
