...
const start = async () => {
  ...
  app.get('/api/world', async (request, reply) => {
    ...
    for (const t of territoryRows) {
      const entry: Record<string, unknown> = {
        id: t.id, ownerId: t.ownerId, hasRoad: t.hasRoad, hasPort: t.hasPort,
      };
  ...
  app.post('/api/action', async (request, reply) => {
    ...
    // actions. Add a matching block for every new action type introduced in Phase 4+.
    if (type === 'build_road') {
      ...
      // Pending-queue check: reject if a build_road is already queued for this territory.
      // Prevents mandate waste on an action the engine would silently discard.
      const alreadyQueued = await prisma.queuedAction.findFirst({
        where: { type: 'build_road', payload: { path: ['territoryId'], equals: p.territoryId } },
      });
      if (alreadyQueued) return reply.code(400).send({ error: 'A road is already queued for this territory' });
    }

    await prisma.$transaction([
      prisma.queuedAction.create({
        data: { nationId, phase: currentPhase(), type, payload: payload as object, tickQueued: meta.tick },
      }),
