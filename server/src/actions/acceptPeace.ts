...
// accept_peace costs 0 Mandate.
const COST = 0;

interface AcceptPeacePayload {
  warId?: number;
}

export const acceptPeaceHandler: ActionHandler = {
  async validate(ctx: ActionContext): Promise<ValidateResult> {
    const p = ctx.payload as AcceptPeacePayload;
    ...
    // Only the NON-proposing party may accept.
    if (deal.proposingNationId === ctx.nationId) {
      return { ok: 'error', status: 400, reason: 'You proposed this deal — wait for the other party to accept or decline' };
    }
