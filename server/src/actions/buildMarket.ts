...
const COST = ACTION_COSTS['build_market']!;
const IND_COST = BUILD_INDUSTRY['market']!;

export const buildMarketHandler: ActionHandler = {
  async validate(ctx: ActionContext): Promise<ValidateResult> {
    ...
    if (ctx.defById.get(p.territoryId)?.isCoastal) return { ok: 'error', status: 400, reason: 'Territory is coastal — build a port instead' };
    if (territory.hasMarket) return { ok: 'error', status: 400, reason: 'Territory already has a market' };
    ...
    if (territory.constructionType !== null) {
      // DEFERRED PATH: mandate + industry pre-charged; market starts when current construction finishes.
      if (ctx.nation.mandateUsed + COST > ctx.myBudget) return { ok: 'error', status: 400, reason: 'Insufficient mandates' };
      await ctx.prisma.$transaction([
        ctx.prisma.territoryState.update({ where: { id: p.territoryId }, data: { pendingConstructionType: 'market' } }),
  ...
  async queue(ctx: ActionContext, cost: number, finalPayload: object): Promise<void> {
    await ctx.prisma.$transaction([
      ctx.prisma.queuedAction.create({
        data: { nationId: ctx.nationId, phase: ctx.currentPhase, type: 'build_market', payload: finalPayload, tickQueued: ctx.currentTick },
      }),
