...
export const declineInstantTradeHandler: ActionHandler = {
  ...
  async queue(ctx: ActionContext, _cost: number, finalPayload: object): Promise<void> {
    ...
    await ctx.prisma.$transaction([
      ctx.prisma.queuedAction.create({
        data: { nationId: ctx.nationId, phase: ctx.currentPhase, type: 'decline_instant_trade', payload: p, tickQueued: ctx.currentTick },
      }),
    ]);
  },
};
